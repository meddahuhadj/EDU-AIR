...
from .confirmation_dialog import ConfirmationCountdownDialog
...
class MainWindow(QMainWindow):
    ...
    def _on_confirm_request(self, intent) -> None:
        if intent is None:
            return
        # Single-instance: a second confirmation request while one is already
        # pending silently supersedes the old prompt (closes it). The Safety
        # gate only routes one sensitive action at a time, so this is safe.
        self._close_confirm_dialog()
        dlg = ConfirmationCountdownDialog(intent, self)
        dlg.destroyed.connect(lambda: self._dialogs.__setitem__("confirm", None))
        dlg.rejected.connect(self.core.reject_pending)
        dlg.confirmed.connect(self.core.confirm_pending)
        self._dialogs["confirm"] = dlg
        dlg.show()
        dlg.raise_()
        dlg.activateWindow()

    def _close_confirm_dialog(self) -> None:
        win = self._dialogs["confirm"]
        if win is not None:
            try:
                win.reject()
            finally:
                self._dialogs["confirm"] = None
