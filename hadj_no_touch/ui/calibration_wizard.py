...
class CalibrationWizard(QDialog):
    def __init__(self, core: AppCore, parent=None):
        ...
        self.btn_skip = QPushButton("Skip (use estimate)")
        self.btn_skip.setToolTip(
            "Skips this point and uses a default camera-position estimate. "
            "Ends with an approximate mapping — recalibrate fully for precision.")
        ...
        try:
            ...
            log.warning("start calibration failed: %s", e)
            self.core.cancel_calibration()
            QMessageBox.warning(
                self, "Calibration",
                "Calibration could not start (camera unavailable?):\n" + str(e))
    ...
    def _update_ui(self, stage: int) -> None:
        estimated = self.core.calibration.estimated if self.core.calibration else set()
        for i, lbl in enumerate(self.stage_labels):
            ...
            if done:
                if est:
                    lbl.setText(f"{ANCHOR_NAMES[i]} (est.)")
                    lbl.setStyleSheet("color: #ffb35c; border: 1px solid #b3802a;"
                                      "border-radius: 6px; padding: 4px 2px; font-size: 11px;")
                else:
                    lbl.setText(ANCHOR_NAMES[i])
                ...
                lbl.setStyleSheet("color: #5f7ea0; border: 1px solid #223d5c;"
                                  "border-radius: 6px; padding: 4px 2px; font-size: 11px;")
        if len(self.core.calibration.estimated) >= 2:
            self.hint.setText(
                (STAGE_TEXTS[stage] + "\n\n⚠ Several points are estimates — the "
                 "mapping will be rough. Recalibrate fully for best precision.")
                if stage < len(STAGE_TEXTS) else
                "Mapped (approximate) — several estimated points. Recalibrate fully "
                "for best precision.")
        elif stage < len(STAGE_TEXTS):
    ...
    def _skip(self) -> None:
        # Real skip: advances the stage with an estimated camera point instead
        # of refusing to do anything. Done-stages are labelled "est." below.
        self.core.skip_calibration_point()
        if self.core.calibration.done:
            self._tick()
