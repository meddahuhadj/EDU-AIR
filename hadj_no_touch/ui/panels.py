...
class SettingsCenterDialog(QDialog):
    def __init__(self, core, parent=None):
        ...
        self.v_dl_guide = QLabel(
            "OFFLINE-FIRST is ON and no Vosk model folder is set, so voice would "
            "be disabled — it will NOT silently fall back to Google. To get local "
            "voice working, download a Vosk model (e.g. vosk-model-small-en-us-0.15 "
            "from https://alphacephei.com/vosk/models) and put its fold path in the "
            "model folder box above.")
        self.v_dl_guide.setWordWrap(True)
        self.v_dl_guide.setStyleSheet("color: #e8c39f;")
        self.v_dl_guide.setVisible(
            self.core.settings.voice.offline_first
            and not str(self.core.settings.voice.vosk_model_path).strip())
        vl.addWidget(self.v_dl_guide)
        vl.addWidget(QLabel(
        ...
        self.v_offline_note = QLabel(
            "OFFLINE-FIRST is ON: voice prefers Vosk so audio stays on this "
            "computer. If no Vosk model folder + `vosk` package are configured, "
            "voice will be disabled with an explicit disclosure — it will never "
            "silently fall back to Google.")
        self.v_offline_note.setWordWrap(True)
        self.v_offline_note.setStyleSheet("color: #9fc7e8;")
        self.v_offline_note.setVisible(self.core.settings.voice.offline_first)
        vl.addWidget(self.v_offline_note)
