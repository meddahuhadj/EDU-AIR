...
class GestureTrainer(QDialog):
    ...
    def _save(self) -> None:
        ...
        verdict = self.core.verify_custom_gesture_samples(list(self._samples))
        if not verdict.get("ok"):
            QMessageBox.warning(
                self, "Not ready to save",
                (verdict.get("reason") or "The movement is not reproducible "
                 "enough across samples yet.") +
                "\n\nRecord a few more consistent samples so the AI can verify it.")
            return
        try:
