...
class GestureSettings:
    ...
    # True only after a discrete click/drag: motion gestures (swipe/scroll)
    # are muted for a beat so a pinch release is never re-read as a swipe.
    click_guard_ms: int = 250
    swipe_scroll_separation_ms: int = 350
    # Movement
    ...
    # Custom gesture fires always route through the Safety Engine; when this
    # flag is on they also demand an explicit confirmation on each trigger.
    custom_gesture_confirmation: bool = True
    custom_gesture_cooldown_ms: int = 1500
    # Sensitive-action confirmation dialog: the Confirm button is disabled for
    # this many milliseconds while an aria-live-style countdown runs, so a
    # reflex gesture can never slam an immediate affirmative through the gate.
    confirmation_countdown_ms: int = 5000
...
class VoiceSettings:
    ...
    offline_first: bool = False  # prefer local (vosk) over online google
    engine_choice_made: bool = False  # first-run chooser shown / accepted
    continuous: bool = True
    push_to_talk: bool = False
    vosk_model_path: str = ""
    wake_word: str = ""  # when set (e.g. "HEY HADJ"), voice only acts on speech
                         # that BEGINS with this phrase; empty means no gate
