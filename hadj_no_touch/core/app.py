...
def _param_num(value: str):
    """int/float coercion for macro/custom-command params, else the raw string."""
    try:
...
class AppCore(QObject):
    ...
    def __init__(self, parent=None):
        ...
        # Persist every safety decision (blocked/confirming/confirmed/rejected/
        # failed) into the activity-log DB, not just the in-memory ring.
        self.safety.on_audit = self._safety_audit_sink
        ...
        # Per-gesture cooldown for custom-gesture fires (name -> monotonic).
        self._custom_fire_lock: dict[str, float] = {}
    ...
    def _execute_intent(self, intent: Intent) -> bool:
        ...
        try:
            ...
            elif a == "OPEN_DOCUMENT":
                return self._open_documents(str(p.get("kind", "")))
    ...
    def _fire_custom_gesture(self, gesture: CustomGesture) -> None:
        """Route a live custom-gesture match through the normal gate.

        Every custom action becomes a real Intent and passes the same
        Authorization pipeline as voice/gesture intents: demo simulation
        (never raw OS calls in demo mode), safety deny-by-default, optional
        confirmation, history and audit. A per-gesture cooldown prevents a
        repeated motion from firing a one-shot action in a burst.
        """
        if not self._can_execute():
            return
        ...
        now = time.monotonic()
        if now - self._custom_fire_lock.get(gesture.name, 0.0) < \
                self.settings.gestures.custom_gesture_cooldown_ms / 1000.0:
            self._event("GESTURE", f"Custom gesture '{gesture.name}' re-armed "
                                   f"(cooldown) — ignored")
            return
        self._custom_fire_lock[gesture.name] = now
        self._event("GESTURE", f"Custom gesture '{gesture.name}' matched")

        params = {}
        if name in ("launch_app", "open_profile", "open_document", "type_text"):
            key = {"launch_app": "app", "type_text": "text",
                   "open_profile": "profile", "open_document": "kind"}[name]
            params[key] = value
        intent_action = {
            "launch_app": "OPEN_APP",
            "type_text": "TYPE_TEXT",
            "open_profile": "PROFILE_SWITCH",
            "open_document": "OPEN_DOCUMENT",
            "screenshot": "SCREENSHOT",
        }.get(name)

        if intent_action is None:
            # Free-form custom intent, e.g. "VOLUME_SET percent=50".
            tokens = action.split()
            intent_action = (tokens[0] or "").upper()
            for tok in tokens[1:]:
                if "=" in tok:
                    k, v = tok.split("=", 1)
                    params[k.strip()] = _param_num(v.strip())

        if not intent_action:
            self._event("WARN", f"Custom gesture '{gesture.name}' has no action "
                                f"assigned — nothing executed")
            return
        # A deliberately fired one-shot: require explicit confirmation on each
        # trigger, unless the user disabled that in settings.
        needs_confirm = self.settings.gestures.custom_gesture_confirmation
        intent = Intent(action=intent_action, params=params, source="custom",
                        description=f"Custom gesture '{gesture.name}' → {intent_action}",
                        needs_confirmation=needs_confirm)
        self._route_intent(intent)

    def _open_documents(self, kind: str = "") -> bool:
        import os
        user = os.environ.get("USERPROFILE", ".")
        raw = (kind or "").strip()
        token = raw.lower().split()[0] if raw else ""
        mapped = {
            "pictures": "Pictures", "images": "Pictures",
            "downloads": "Downloads", "music": "Music", "videos": "Videos",
            "documents": "Documents",
        }.get(token)
        if mapped:
            folder = os.path.join(user, mapped)
        elif os.path.isabs(raw):
            folder = raw
        else:
            folder = os.path.join(user, "Documents")
        ...
        try:
            ...
            self._event("ERROR", f"open documents failed: {e}")
            return False
    ...
    def skip_calibration_point(self) -> None:
        """Skip the current stage without a real fingertip sample.

        The manager substitutes a default camera-position estimate for the
        skipped anchor and flags it, so the homography can still be built but
        is honestly reported as approximate.
        """
        self.calibration.skip_point()
        if self.calibration.done:
            self._finish_calibration()
    ...
    def _finish_calibration(self) -> None:
        ...
        est = len(self.calibration.estimated)
        if est:
            self._event("WARN", f"Calibration complete with {est} estimated "
                                f"point(s) — mapping is approximate")
            self.toasts.emit("Calibration saved (some points estimated). "
                             "Recalibrate fully for best precision.")
        else:
    ...
    def _save_calibration(self) -> None:
        ...
        self.db.save_calibration(self.calibration.points_camera, h_list,
                                 estimated=sorted(self.calibration.estimated))
    ...
    def _load_calibration(self) -> None:
        ...
        if rec and rec.get("points") and len(rec["points"]) >= 4:
            ...
            self.calibration.estimated = set(rec.get("estimated") or [])
    ...
    # custom gestures -----------------------------------------------------
    def verify_custom_gesture_samples(self, samples: list[list[np.ndarray]]) -> dict:
        """Test-then-assign dry-run for a freshly taught custom gesture.

        Replays every recorded sample through the *same* resampling + distance
        machinery the live matcher uses, and returns an honest verdict about
        whether the movement is actually learnable before an action is bound.
        Nothing is persisted and no action fires here — this is a pure dry-run.

        Returns ``{ok, reason, pair_distances, mean_distance, worst_pair}``.
        """
        from ..gestures.custom_gestures import resample_path, path_distance, MATCH_THRESHOLD
        pair_dists: list[float] = []
        templates = [resample_path(path) for path in samples if len(path) >= 2]
        for i in range(len(templates)):
            for j in range(i + 1, len(templates)):
                pair_dists.append(float(path_distance(templates[i], templates[j])))
        if not pair_dists:
            return {"ok": False,
                    "reason": "At least 2 valid samples are required.",
                    "pair_distances": [], "mean_distance": 0.0, "worst_pair": 0.0}
        worst = float(max(pair_dists))
        ok = worst <= MATCH_THRESHOLD
        reason = "" if ok else (
            f"Movement is too inconsistent across samples "
            f"(worst template distance {worst:.2f} exceeds live match threshold "
            f"{MATCH_THRESHOLD:.2f}). Record more consistent samples.")
    ...
    def _safety_audit_sink(self, rec: dict) -> None:
        """Persist a Safety-gate decision to the activity DB.

        Called from the Safety Engine's ``on_audit`` hook after every
        allow/block/strike/re-arm decision, so the confirmation-dialog fixes
        are not merely in-memory: sensitive-action outcomes leave a durable,
        ordered trail the Test Lab can inspect after the fact.
        """
        try:
            ...
            # Cleanup must never break the interaction loop.
            log.error("safety audit sink failed", exc_info=exc)
