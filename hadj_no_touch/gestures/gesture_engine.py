...
class GestureEngine:
    ...
    def __init__(self, settings: GestureSettings | None = None,
        ...
        # Motion-defence markers (monotonic deadlines). A discrete click locks
        # out swipes/scrolls/palm-hold for a beat; swipes and scrolls lock each
        # other out so one flick can never be read as two competing actions.
        self._post_click_until = 0.0
        self._post_swipe_until = 0.0
        self._post_scroll_until = 0.0
        ...
        # False-trigger guardrails:
        #  - _post_click_until: right after a discrete click/drag, fast motion
        #    poses must not be re-read as a swipe/scroll/palm-hold/circle.
        #  - _post_swipe_until / _post_scroll_until: swipe and scroll are
        #    mutually exclusive for a beat so one motion never double-fires.
        self._post_click_until = 0.0
        self._post_swipe_until = 0.0
        self._post_scroll_until = 0.0
    ...
    def _note_click_marker(self) -> None:
        """Mute motion gestures for a short window after a discrete click/drag,
        so the pinch release / gesture transition is never re-read as another
        motion gesture on the way out."""
        now = time.monotonic()
        self._post_click_until = now + self.settings.click_guard_ms / 1000.0

    def _motion_guard_ok(self, now: float, kind: str) -> bool:
        """True only if the given motion-gesture kind may fire right now."""
        if now < self._post_click_until:
            return False
        if kind == SCROLL_V and now < self._post_swipe_until:
            return False
        if kind in (SWIPE_LEFT, SWIPE_RIGHT, SWIPE_UP, SWIPE_DOWN) \
                and now < self._post_scroll_until:
            return False
        return True
    ...
    def update(self, hands: list[HandData], pointer: Optional[tuple[float, float]],
        ...
        if g in (OPEN_PALM, POINT, TWO_FINGER) and res.confidence > cfg.gesture_confidence \
                and self._motion_guard_ok(now, CIRCLE_CW):
        ...
        if g == OPEN_PALM:
            ...
            if scroll_ev:
                ...
                if self._motion_guard_ok(now, PALM_HOLD) \
                        and self._can_fire(PALM_HOLD, cfg.gesture_cooldown_ms):
    ...
    def _check_swipe(self, res: GestureResult, confirmed: str) -> list[GestureEvent]:
        cfg = self.settings
        now = time.monotonic()
        if not self._motion_guard_ok(now, SWIPE_LEFT):
            return []
        ...
        if not self._can_fire(kind, 900) or not self._motion_guard_ok(now, kind):
                return []
        self._positions.clear()
        self._post_swipe_until = time.monotonic() + \
            self.settings.swipe_scroll_separation_ms / 1000.0
    ...
    def _scroll_from_motion(self):
        ...
        if abs(dy) < thr:
            return None
        if not self._motion_guard_ok(now, SCROLL_V):
            return None
        if not self._can_fire(SCROLL_V, cfg.scroll_gesture_cooldown_ms):
            return None
        self._post_scroll_until = time.monotonic() + \
            self.settings.swipe_scroll_separation_ms / 1000.0
