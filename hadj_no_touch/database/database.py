...
CREATE TABLE IF NOT EXISTS calibration (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    points TEXT, homography TEXT, created REAL, label TEXT, estimated TEXT
);
...
class Database:
    def __init__(self, path: Path | None = None):
        ...
        with self._lock:
            ...
            try:
                # Migration: older databases lack the estimated column.
                self._conn.execute(
                    "ALTER TABLE calibration ADD COLUMN estimated TEXT")
            except Exception:
                pass
            self._conn.commit()
    ...
    # ---- calibration -----------------------------------------------------------
    def save_calibration(self, points: list, homography: list | None,
                         label: str = "main", estimated: list | None = None) -> int:
        with self._lock:
            cur = self._conn.execute(
                "INSERT INTO calibration(points, homography, created, label, estimated)"
                " VALUES (?,?,?,?,?)",
                (json.dumps(points), json.dumps(homography) if homography else None,
                 time.time(), label,
                 json.dumps(list(estimated or []))),
            )
            self._conn.commit()
    ...
    def latest_calibration(self) -> Optional[dict]:
        with self._lock:
            row = self._conn.execute(
                "SELECT points, homography, created, estimated"
                " FROM calibration ORDER BY created DESC LIMIT 1"
            ).fetchone()
        if not row:
            return None
        est = row[3]
        try:
            estimated = json.loads(est) if est else []
        except Exception:
            estimated = []
        return {
