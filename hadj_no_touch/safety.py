...
PREDEFINED_ACTIONS: list[ActionSpec] = [
    ...
    _safe("OPEN_DOCUMENT", "Open the documents folder", "app"),
...
class SafetyEngine:
    ...
    def __init__(self, registry: ActionRegistry | None = None,
        ...
        self._max_audit = 400
        # Optional sink (typically -> the activity-log DB) for every audit call.
        # The core sets this so blocked/confirmed/rejected outcomes survive
        # the in-memory ring buffer and land in the persistent activity log.
        self.on_audit: callable | None = None
