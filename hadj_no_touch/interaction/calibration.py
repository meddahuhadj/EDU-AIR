...
# Best-effort camera-space defaults for a typical desk webcam, used ONLY when
# the user explicitly skips a calibration point. The result is honestly
# reported as an approximate mapping, never a precise one.
...
class CalibrationManager:
    ...
    # Indices of stages whose camera point was skipped/estimated, so the UI
    # can label them and the user knows the mapping is approximate.
    ...
    def skip_point(self) -> None:
        """Skip the current stage using a default camera-position estimate.

        The skipped stage is recorded in ``estimated`` so callers can warn the
        user that the resulting mapping is approximate rather than precise.
        """
        if self.done:
            return
        stage = self.stage
        placeholder = DEFAULT_CAMERA_POINTS[stage] if stage < len(DEFAULT_CAMERA_POINTS) \
            else (0.5, 0.5)
        self.points_camera.append(placeholder)
        self.estimated.add(stage)
        self.stage += 1
        self.point_samples.clear()
        self._try_build(silent=True)
