...
- **Alternate/other host (same static landing only):** `netlify.toml` +
  `web/_headers` publish the identical `web/` folder to **Netlify** — no other
  account needed, drop the folder on Netlify Drop or push the repo with the
  toml present. Tests `tests/test_netlify_deploy.py` gate parity with Vercel.
- **Honest scope of EVERY deployment above:** it is the *static landing page
  only*. The HADJ AI itself is a local desktop application — it uses your
  camera, microphones and runs on your own machine; it is **never** deployed,
  hosted or shipped to any link. No Netlify, Vercel or other host has ever
  received the AI code or will.
  immediately
